...
- **dashboard**（本地网页）：总览、群组勾选、喊单人战绩、代币列表、交易与持仓、24h 战况（累计盈亏曲线 / 收益分布 / 按链按群按人 / 错过的金狗）、复盘（首喊后 24h 达标的币与其余相比，哪些首喊时就能看到的群 / 推特 / 买入人特征更常见；逐币事件时间线）、模拟交易（按选定特征假想买入、按止盈 / 止损 / 回撤 / 时限假想卖出，与基准并排）、面板设置。
...
dashboard「复盘」页：首喊在过去 3 天（可切 24h / 7d / 30d）的币，按**首喊后 24h 峰倍 ≥ 2×**（可切 3× / 5×）分「达标 / 未达标」，比较三类特征在两组里的出现率——群（基准市值、首喊人的历史达标率…）、推特（有官推、粉丝、账号年龄、蓝标、截面前的推文 / GMGN 喊单…）、买入人（持有人数、fomo 关注者买入 / 持有、前排比例、前 10 钱包占供应…）。特征只取**首喊后 30 秒**那一刻能看到的信息（防止拿涨完以后的粉丝数解释为什么涨）；首喊之后才发生的（30 分钟内几人喊、官推发推、fomo 买入）单独列为「过程」。点任一币展开事件时间线，以首次 ≥1.5× 为界分「之前 / 之后」。

这是关联统计不是因果：样本只有被监听群喊过的币；两组任一不到 20 个的特征只报计数。带截面的特征从装上这一版起才开始积累，前几天推特 / 买入人两类多为「样本不足」。页面右上「复制 Markdown」把整份报告（特征对照 + 逐币时间线）复制到剪贴板，方便贴给别人或别的模型讨论。

### 模拟交易

侧栏「模拟交易」页：策略是一段 JS。`entry(t)` 决定买不买、买多少（`t.mc`、`t.card`、`t.prior`、`t.ageSec` / `t.openSec`（首喊距代币创建 / 迁出多久）、`t.twitter.isTweet`（推特链接指向一条推文而非账号）……只有喊单那一刻能看到的东西；返回美元数就是这单的仓位；返回 `"watch"` 则先看着，之后在 `step` 里 `s.buy()` 按走势再买——有选择地买），`step(s)` 每 20 秒采样调一次决定卖不卖（`s.x` 当前倍数、`s.peakX` / `s.dd` 最高与回撤、`s.held` 持有秒数、`s.events.calls` 到现在为止哪些群喊了、`s.state` 记状态、`s.token` 就是 `entry` 收到的那个 `t`；`s.sell(0.5, "出本")` / `s.sellAll("止损")`，tag 就是退出原因）。页面上 8 个模板——2× 全清、2× 出本尾仓至今、出本后保本、阶梯 2/4/8×、回撤止盈、第二个群喊了、挑币 + 加仓、什么都不卖——点了就是完整可跑的代码，改完 ⌘↵ 运行；点特征 chip 把条件插到光标处；出错定位到行，`console.log` 有面板。结果：交易数、净盈亏（拆已实现 / 未实现、去掉最好一笔）、胜率、退出原因（按你的 tag）、累计曲线、**调参段 vs 验证段**（最后 N 天只看不调，两段都正才算数）、按日表、逐笔（盈利的那些带买入后倍数路径）；同一套卖法不挑币的「全部合格币」作基准。策略跑在 sidecar 的 worker 线程里，15 秒超时；不是安全沙箱，只跑自己写的。不下真单、不进账本；成交按 20 秒采样价、不计对小池子的冲击。
